...
if(isset($_POST['btnSave']))
{
		...
		$result ="SELECT name,e_id FROM admin where e_id='$up_by'";
		$tt=mysql_query($result);
		while($row = mysql_fetch_array($tt))
        {
			$up_name=$row['name'];
	...
	{	
	   ...
	   if($fg==0)
	   {
		$query = "INSERT INTO notices VALUES ('$nid','$title','$cat_name','$cat_no','$today','$up_by','$up_name','$newfilename ','$texts','')";
         mysql_query($query);
...
<tr>
            ...
            <tr>
				<td><label for="pass"><b>Text: </b></label></td>
				<td>
                <textarea cols="50" rows="8" name="texts">
                </textarea>
                </td>
			</tr>
			<tr>
