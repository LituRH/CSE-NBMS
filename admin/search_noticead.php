...
if($n_cat_no==1)
  {
	 $searchid=$sdat ; 
	$query=" SELECT * FROM notices WHERE n_pub='$searchid' ORDER BY id DESC limit $eu, $limit ";

	}
	else 
	{
	  $searchid=$n_cat_no ;
	  $query=" SELECT * FROM notices WHERE n_cat_no='$searchid' ORDER BY id DESC limit $eu, $limit ";
	}
foreach ($dbo->query($query) as $row) {
$m=$i%2; // required for alternate color of rows matching to style class 
$i=$i+1;   //  increment for alternate color of rows
echo "<tr class='r$m'><td>$row[0]</td><td><a href=page2ad.php?id=$row[0]>$row[1]</a></td><td>$row[2]</td><td>$row[4]</td><td>$row[5]</td><td><a href=deletenotice_admin.php?id=$row[0]>Delete</a>/<a href=updatenotice_admin.php?id=$row[0]>Update</a></td></tr>";
}
